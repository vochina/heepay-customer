...
class HeepayCustomer
{
    ...
    public function sign($content): string
    {
        ...
        $sign = $sm2->doSign($content, $key[1], '1234567812345678');
        return self::asn1_to_rs($sign);
    }

    public function verifySign($content, $sign): bool
    {
        $sm2 = new RtSm2('base64', true);
        $sign = self::rs_to_asn1($sign);
        $pem = MyAsn1::decode(file_get_contents($this->sm2publicKeyPath), 'base64');
        return $sm2->verifySign($content, $sign, $pem[1], '1234567812345678');
    }
